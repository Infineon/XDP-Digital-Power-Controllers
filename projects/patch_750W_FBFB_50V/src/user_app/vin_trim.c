...
//		PMBUS_Set_Command_Dword(page, PMBUS_CMDCODE_MFR_DBG_1,user_data.active_vin_trim_slope);
	}
#endif
...
//		PMBUS_Set_Command_Dword(page, PMBUS_CMDCODE_MFR_DBG_2,user_data.standby_vin_trim_slope);
	}
#endif
...
{
	// check if user variable for setting is not empty if empty use otp setting
	if(slope != 0 || offset != 0)
	{
...
//		PMBUS_Set_Command_Dword(0, PMBUS_CMDCODE_MFR_DBG_1, (slope << 16 | offset));
//		PMBUS_Set_Command_Dword(1, PMBUS_CMDCODE_MFR_DBG_1, (slope << 16 | offset));
	}
}
#endif
