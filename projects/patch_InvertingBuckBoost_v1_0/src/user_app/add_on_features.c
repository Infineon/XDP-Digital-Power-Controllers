...
{
	user_data.en_buck_boost_feed_forward =
			Regulation_Get_PMBUS_Configuration(PMBUS_PAGE_0, REGULATION_CONFIG_EN_BOOST_FEED_FORWARD);

	// to get user's MFR_IOUT_OFFSET_CORR_FACTOR value
	user_data.iout_telem_offset_correction_slope_factor_exp = -8;
	PMBUS_HANDLE_MFR_IOUT_OFFSET_CORR_SLOPE_FACTOR(PMBUS_PAGE_0, PMBUS_WRITE);
	user_data.iout_telem_offset_correction_factor_exp = -1;
	PMBUS_HANDLE_MFR_IOUT_OFFSET_CORR_FACTOR(PMBUS_PAGE_0, PMBUS_WRITE);



#ifdef eff_table_feature_en
	//================================
	// For efficiency table feature,
	// set your input voltage hysteresis
	// and output current hysteresis
	// accordingly
	user_data.vin_hysteresis = 1;	// LSB = 1V
	user_data.iout_hysteresis = 3;	// LSB = 0.25A
	//================================

	// Do not change
	// initial input voltage threshold
	user_data.vin_p1_adjust = 0;
	user_data.vin_p2_adjust = 0;
	// initial output current threshold
	user_data.iout_p1_adjust = 0;
	user_data.iout_p2_adjust = 0;
	user_data.iout_p3_adjust = 0;
    user_data.iout_p4_adjust = 0; //220504 changes

#endif

user_data.iout_exp[PMBUS_PAGE_0] = (int8_t)TWOS_UNCOMPLEMENT(4,
		Telemetry_Get_PMBUS_Configuration(PMBUS_PAGE_0,
				(TELEMETRY_CONFIG)TELEMETRY_CONFIG_READ_IOUT_EXP));

#ifdef zvs_feature_en
	//================================
	// ZVS feature settings
	user_data.iout_zvs_hysteresis = 8;	// LSB = 0.125 A
	user_data.iout_zvs_hysteresis_exp = -3;

	PMBUS_HANDLE_MFR_ZVS_DISABLE_THRESHOLD(PMBUS_PAGE_0, PMBUS_WRITE);
	//================================
#endif

#ifdef light_load_features_en
	//================================
	// light_load_features settings
	user_data.light_load_features_hysteresis = 16;	// LSB = 0.125 A
	user_data.light_load_features_hysteresis_exp = -3;

	PMBUS_HANDLE_MFR_LIGHT_LOAD_MODE(PMBUS_PAGE_0, PMBUS_WRITE);
	PMBUS_HANDLE_MFR_LIGHT_LOAD_THRESH(PMBUS_PAGE_0, PMBUS_WRITE);

	PMBUS_HANDLE_MFR_BURST_CONFIG(PMBUS_PAGE_0, PMBUS_WRITE);
	//================================
#endif
	...
	user_data.zvs_current_value = 0;

#ifdef deadtime_feature_en
	PMBUS_HANDLE_MFR_DEADTIME(PMBUS_PAGE_0, PMBUS_WRITE);
#endif

#ifdef vin_trim_feature_en
	PMBUS_HANDLE_MFR_VIN_TRIMMING_STANDBY(PMBUS_PAGE_0, PMBUS_WRITE);
	PMBUS_HANDLE_MFR_VIN_TRIMMING_ACTIVE(PMBUS_PAGE_0, PMBUS_WRITE);
#endif
