...
extern PROTECTION_t protection_data;
#endif
...
#ifdef uart_debug
	// use together with uart debug Now add new code here:
	if (direction == PMBUS_WRITE)
	{
		// get the command code here:
...
#ifdef uart_debug
/**
 * Example of callback function on completion of UART writes.
 * Here we write to a scratch register RGU_SPARE_FF different
 * values based on the status received from the UART driver.
 * We use this callback to create a loopback.
 */
void cb_test (uint32_t event)
{
	// write a spare register on event callbacks for UVM testing of callbacks
	RGU_SPARE_FF__SET(0);
	if ((event & ARM_USART_EVENT_RX_BREAK) == ARM_USART_EVENT_RX_BREAK)
	{
		RGU_SPARE_FF__SET(0xBBBBBBBB);  // write the event to a register for waves debug
	}
	else if ((event & ARM_USART_EVENT_RX_FRAMING_ERROR) == ARM_USART_EVENT_RX_FRAMING_ERROR)
	{
		RGU_SPARE_FF__SET(0xFFFFFFFF);
	}
	else if ((event & ARM_USART_EVENT_RX_PARITY_ERROR) == ARM_USART_EVENT_RX_PARITY_ERROR)
	{
		RGU_SPARE_FF__SET(0xCCCCCCCC);
	}

	// this part performs the loopback mechanism by calling Send and Receive again
	if ((event & ARM_USART_EVENT_RECEIVE_COMPLETE) == ARM_USART_EVENT_RECEIVE_COMPLETE)
	{

		for (uint32_t i = 0; i < (N_ROWS); i++)
		{
			src_buf[i] = dest_buf[i];
		}
		int32_t status_tx = UARTdrv->Send(&src_buf[0], N_ROWS);
		int32_t status_rx =	UARTdrv->Receive(&dest_buf[0],N_ROWS);
		if (((status_tx | status_rx) != DRIVER_OK))
		{
			USER_Log_Error(PROGRAM_COUNTER, PMBUS_STATUS_CML_PROCESSOR_FAULT);
		}
	}
