...
typedef struct {
	...
	uint32_t fsw[NUM_LOOPS];					//!< current telemetry value for switching frequency of each loop
...
void AT_FAULT_SHUTDOWN(PMBUS_PAGE_t loop); 								//Ivan addition
void AT_PRESTARTUP(PMBUS_PAGE_t loop);  								//VenkatG addition
