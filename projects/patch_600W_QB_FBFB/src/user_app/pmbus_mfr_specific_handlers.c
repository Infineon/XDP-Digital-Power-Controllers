 ...
 * [7:4] > step rate.  The number of 16 fsw units to wait between steps
...
void PMBUS_HANDLE_MFR_FREQUENCY_DITHER(PMBUS_PAGE_t page, PMBUS_DIRECTION_e direction)
{
	if ((direction == PMBUS_WRITE)||(direction == OTP_RESTORE))
	{
		...
		// grab the current frequency set point and calculate the min and max values of the triangle wave
		...
		//Actual task is here, adding/removing delta to the period
